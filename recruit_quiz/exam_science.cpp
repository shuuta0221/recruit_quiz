...
QuestionList CreatePhysicsExam()
{
    ...
    {//重力加速度
        ...
        questions.push_back({ "重力加速度を9.8m/s^2とする。\n初速" + to_string(v0) + "m/sで物体を鉛直に投げ上げたとき、" + to_string(t) +
            "秒後の物体の速度はXm/sである。\nXの値を小数点以下第1位まで求めよ(空気抵抗はないものとする)。",answer });
    }//重力加速度
    ...
    {//浮力
        ...
        if (v % 10) {   //小数点以下第1位が0以下なら、小数部を文字にする
            answer += '.';
            answer += '0' + v % 10;
        }
    ...
    {//ばね
        ...
        if (k % 10) {                       //小数点以下第1位が0以下なら、小数部を文字列に加える
            answer += '.';
            answer += '0' + k % 10;
        }
    ...
    {//振り子
        ...
        questions.push_back({ "長さ" + to_string(l * 1) + "mの糸に質量" + to_string(m) + "kgの重りをつけ、わずかに揺らしたところ、周期はX秒であった。Xの値を求めよ。\n" +
        ...
        questions.push_back({ "重力加速度を10m/s^2とする。\n長さ" + to_string(l) + "mの糸に質量" + to_string(m) + "kgのおもりをつけた振り子がある。\nこのおもりを最下点から高さ" +
