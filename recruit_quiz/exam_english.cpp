...
QuestionList CreateEnglishPhraseExam()
{
	const struct {
		...
		const char* exanple;		//英語の例文
		const char* translation;	//例文の日本語訳
		int	blankCount;				//穴の候補の数
		int	blankOptions[3];		//穴の候補になる単語の位置
	} data[] {
		{"too [A] to [B]","とても[A]なので[B]","The tea was too hot to drink.","お茶が熱すぎて飲めなかった。",2,{3,5}},
	...
	case 0:	//空欄に入る単語を答える
		for (int i = 0; i < quizCount; i++) {
			...
			//例文を単語に分解
			vector<string> words;
			const char* start = e.exanple;
			for (const char* p = e.exanple; *p; p++) {
				if (*p == ' ') {
